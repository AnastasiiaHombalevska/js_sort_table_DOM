...
function sortingData(columnNumber) {
    ...
    rows.sort((item1, item2)=>{
        ...
        const columnHeader = document.querySelector("thead").querySelectorAll("th")[columnNumber].textContent.trim().toLowerCase();
        const isNumeric = !isNaN(cell1) && !isNaN(cell2);
        const isSalary = columnHeader === "salary";
        if (isSalary) return parseFloat(cell1.slice(1)) - parseFloat(cell2.slice(1));
